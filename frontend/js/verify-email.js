...
function mostrarAguardando() {
  ...
  verifyTitle.textContent = 'A aguardar confirmação'
  verifyText.textContent = userEmail
    ? `O registo de ${userEmail} foi criado. Abre o link mais recente que recebeste por email para concluir a ativação.`
    : 'Estamos a aguardar a confirmação do teu email. Abre o link recebido para concluir a ativação.'

  statusMessage.className = 'status-message loading'
  statusText.innerHTML = '<span class="spinner"></span><span>Conta criada. Falta apenas confirmar o email.</span>'
  loadingText.textContent = 'Se o email não chegou, usa o botão abaixo para pedir novo envio.'
...
function mostrarErro(mensagem) {
  ...
  verifyTitle.textContent = 'Não foi possível validar a conta'
...
async function verificarEmail() {
  try {
    ...
    if (!user) {
      ...
      mostrarErro('Não existe sessão valida neste browser. Usa o link mais recente do email ou faz login novamente.')
      return
    }
...
resendEmailButton.addEventListener('click', async () => {
  if (!userEmail) {
    showToast('Não existe email pendente para reenvio.', { type: 'error' })
    return
  }
  ...
  if (resultado.sucesso) {
    statusMessage.className = 'status-message success'
    statusText.textContent = 'Novo email de confirmação enviado com sucesso.'
    showToast('Email reenviado com sucesso.', { type: 'success' })
  } else {
    statusMessage.className = 'status-message error'
    statusText.textContent = `Falha no reenvio: ${resultado.erro}`
    showToast('Não foi possível reenviar o email.', { type: 'error' })
  }
