...
function cacheDom() {
  ...
  ui.detailedStats = document.getElementById('profile-detailed-stats')
...
async function carregarPerfil() {
  try {
    ...
    const [seguidores, publicacoes, mensagens, estatisticas] = await Promise.all([
      ...
      obterEstatisticasUser(state.usuario.id)
    ])
...
function renderDetailedStats() {
  ...
  ui.detailedStats.innerHTML = rows.map(([label, value]) => `
...
function renderHero() {
  ...
  const bio = state.perfil?.bio?.trim() || 'Adiciona uma bio na configuração para personalizar esta área.'
...
function renderXpSummary() {
  ...
  ui.xpLevel.textContent = `Nível ${xp.nivel_xp}`
  ...
  ui.xpMeta.textContent = xp.proximo_nivel
    ...
    : `${xp.xp_total} XP total / nível máximo`
}
...
function renderPublicacoes() {
  ...
  if (!state.publicacoes.length) {
    ui.publicationsList.innerHTML = '<article class="profile-empty-card"><p>Ainda não tens publicações visíveis.</p></article>'
    return
  }
  ...
  ui.publicationsList.innerHTML = state.publicacoes.map((publicacao) => `
    <article class="profile-publication-card">
      ...
      <p>${escapeHtml(publicacao.descricao || 'Sem descrição adicional.')}</p>
...
function renderSeguidores() {
  ...
  if (!state.seguidores.length) {
    ui.followersList.innerHTML = '<article class="profile-empty-card"><p>Ainda não tens seguidores visíveis.</p></article>'
    return
  }
...
function renderMensagens() {
  ...
  if (!state.mensagens.length) {
    ui.messagesList.innerHTML = '<article class="profile-empty-card"><p>Ainda não tens mensagens.</p></article>'
    return
  }
...
async function onMessageSubmit(event) {
  ...
  if (!state.usuario) {
    showMessageFeedback('Sessão inválida. Faz login novamente.', 'error')
    return
  }
  ...
  if (!recipientEmail || !content) {
    showMessageFeedback('Preenche o email do destinatário e a mensagem.', 'error')
    return
  }

  if (recipientEmail === String(state.usuario.email || '').trim().toLowerCase()) {
    showMessageFeedback('Não podes enviar mensagem para a tua própria conta.', 'error')
    return
  }

  const recipient = await obterPerfilPorEmail(recipientEmail, state.usuario.id)
  if (!recipient) {
    showMessageFeedback('Não foi encontrado nenhum utilizador com esse email.', 'error')
    return
  }

  const enviada = await enviarMensagem(state.usuario.id, recipient.id, content)
  if (!enviada) {
    showMessageFeedback('Não foi possível enviar a mensagem. Verifica se a política de mensagens já foi aplicada no Supabase.', 'error')
    return
  }
