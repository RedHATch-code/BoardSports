...
async function carregarLeaderboard() {
  ui.list.innerHTML = '<article class="xp-empty-card"><p>A carregar ranking...</p></article>'
...
function renderLeaderboard() {
  ...
  if (!riders.length) {
    ui.list.innerHTML = `
      <article class="xp-empty-card">
        <p>Ainda não há XP registado. Aplica o SQL do BoardSports XP System e valida submissões para popular o ranking.</p>
      </article>
    `
    return
  }
  ...
  ui.list.innerHTML = riders.map((rider, index) => {
    ...
    return `
      <article class="leaderboard-card${topClass}" data-rank="${rank}">
        <div class="leaderboard-rank">
          <span>${getRankMedal(rank)}</span>
        ...
        <div class="leaderboard-score">
          ...
          <span>${getXpProgress(summary)}% para o próximo nível</span>
          <i class="leaderboard-xp-meter" style="--xp-progress:${getXpProgress(summary)}%"></i>
        </div>
      </article>
    `
  }).join('')
...
function getXpProgress(summary) {
  const progress = Number(summary?.progresso_percentagem ?? 0)
  if (!Number.isFinite(progress)) return 8
