...
async function inicializarConfiguração() {
  const autenticado = await inicializarPaginaProtegida()
  if (!autenticado) return

  cacheDom()
  bindEvents()
  await carregarConfiguração()
}
...
async function carregarConfiguração() {
  try {
...
function renderHero() {
  const displayName = ui.nome.value.trim() || state.perfil?.nome || state.usuario?.email || 'Configuração'
  ui.heroName.textContent = `Configuração de ${displayName}`
  ui.heroCopy.textContent = 'Atualiza foto, bio, dados de contacto e segurança num só lugar.'
...
function onPhotoSelected(event) {
  ...
  if (!file.type.startsWith('image/')) {
    updatePhotoMeta('Seleciona um ficheiro de imagem válido.')
    event.target.value = ''
    return
  }
...
function onRemovePhoto() {
  ...
  renderAvatar('')
  updatePhotoMeta('A foto será removida quando guardares o perfil.')
}
...
async function onSubmit(event) {
  ...
  if (!state.usuario) {
    showInlineMessage('Sessão inválida. Faz login novamente.', 'error')
    return
  }
...
document.addEventListener('DOMContentLoaded', inicializarConfiguração)
