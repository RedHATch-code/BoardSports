...
function writeStorage(key, value) {
  try {
    ...
    console.warn('Não foi possível guardar estado local de auth:', error)
  }
}
...
export async function processarRedirecionamentoAuth() {
  try {
    ...
    return { sucesso: false, erro: error.message || 'Não foi possível processar o link de autenticação.' }
  }
}
...
export async function garantirPerfilUtilizador(user = null, fallbackProfile = {}) {
  try {
    ...
    const existingProfile = await obterPerfilCru(currentUser.id).catch((error) => {
      console.warn('Não foi possível obter o perfil antes do upsert:', error)
      return null
    })
  ...
  try {
    ...
    if (normalizedMessage.includes('email not confirmed')) {
      ...
      return {
        ...
        erro: 'Este email ainda não foi confirmado. Abre o link que recebeste no email ou pede um novo envio na página de verificação.'
      }
    }
    ...
    return { sucesso: false, erro: rawMessage || 'Não foi possível iniciar sessão.' }
  }
}
...
export async function fazerRegistro(email, password, tipoUser, dadosAdicionais = {}) {
  try {
    ...
    return {
      ...
      confirmaçãoAindaAtiva: !data.session
    }
  } catch (error) {
    const rawMessage = error.message || ''
    const normalizedMessage = rawMessage.toLowerCase()
    ...
    if (normalizedMessage.includes('sending confirmation email')) {
      return {
        ...
        erro: 'Não foi possível enviar o email de confirmação. Confirma se o envio de emails do Supabase está disponível para o projeto.'
      }
    }

    return { sucesso: false, erro: rawMessage || 'Não foi possível criar a conta.' }
  }
}
...
export async function pedirRecuperacaoPassword(email) {
  try {
    ...
    if (
      ...
      return {
        sucesso: false,
        codigo: 'recovery_email_delivery_failed',
        erro: 'Não foi possível enviar o email de recuperação. Confirma no Supabase se o SMTP personalizado está ativo e se o remetente está verificado.'
      }
    }
    ...
    return {
      ...
      erro: rawMessage || 'Não foi possível enviar o email de recuperação.'
    }
  }
}
