import { obterGaleriaVídeosSpots, obterModalidades } from './db_utils.js'
...
async function initVídeosPage() {
  ...
  bindEvents()
  await Promise.all([loadModalidades(), loadVídeos()])
}
...
async function loadVídeos() {
  state.allVideos = await obterGaleriaVídeosSpots(
...
function renderShortVídeos() {
  const videos = getVideosByFormat('shorts')

  if (!videos.length) {
    ui.shortsFeed.innerHTML = `
      <article class="videos-empty-card">
        <h2>Ainda não há sessões verticais.</h2>
        <p>Publica um vídeo num spot para alimentar o feed da comunidade.</p>
        <a class="map-primary-button has-ui-icon icon-map" href="mapa.html">Abrir mapa</a>
      </article>
    `
    return
  }
      ...
      <article class="videos-short-card">
        ...
        <div class="videos-short-overlay">
          ...
          <div>
            ...
            <p>${escapeHtml(video.legenda || 'Sem legenda adicional para este vídeo.')}</p>
            <small>${escapeHtml(formatDate(video.data_criacao))}</small>
          </div>
          <div class="videos-card-actions videos-short-actions">
            <a class="has-ui-icon icon-map" href="${escapeHtml(mapaHref)}">Mapa</a>
            <a class="has-ui-icon icon-open" href="${escapeHtml(video.video_url)}" target="_blank" rel="noreferrer">Original</a>
          </div>
        </div>
      </article>
    `
...
function renderLongVídeos() {
  const videos = getVideosByFormat('longs')

  if (!videos.length) {
    ui.grid.innerHTML = `
      <article class="videos-empty-card">
        <h2>Ainda não há vídeos longos.</h2>
        <p>Abre um spot, publica uma sessão e liga o vídeo ao local certo.</p>
        <a class="map-primary-button has-ui-icon icon-map" href="mapa.html">Abrir mapa</a>
      </article>
    `
    return
    ...
    return `
      <article class="videos-card">
        ...
        <div class="videos-card-meta">
          ...
          <span>Modalidade: ${escapeHtml(modalidade)}</span>
        </div>

        <div class="videos-card-actions">
          <a class="has-ui-icon icon-map" href="${escapeHtml(mapaHref)}">Abrir no mapa</a>
          <a class="has-ui-icon icon-open" href="${escapeHtml(video.video_url)}" target="_blank" rel="noreferrer">Abrir original</a>
        </div>
      </article>
    `
  }).join('')
...
function buildVideoMediaMarkup(url, title, layout = 'long') {
  ...
  return `
    <div class="videos-card-fallback">
      <div>
        <span class="videos-card-tag">Link externo</span>
        <strong>${escapeHtml(title)}</strong>
        <p>Este vídeo não suporta pré-visualização direta. Abre o original para ver a sessão.</p>
      </div>
    </div>
  `
}
...
document.addEventListener('DOMContentLoaded', initVídeosPage)
